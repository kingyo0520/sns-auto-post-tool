...
1. このディレクトリの内容でGitHubリポジトリを作成
   - **注意**:GitHub Pagesは無料プランではパブリックリポジトリでのみ利用可能なため、このリポジトリは**パブリック**にする必要があります。コード自体に機密情報は含まれない設計(トークン類はすべてGitHub Secretsで管理)なので公開しても問題ありません
...
運用ルールとして、平日は朝7時・夕方17時の1日2回、土日は朝7時のみ投稿する想定です。
1日に複数回チェックが走るため、**「投稿予定日」には日付だけでなく時刻まで設定してください**
(Notionの日付プロパティで「時刻を含める」をオンにする)。

- 朝の投稿枠 → 投稿予定日を「07:00」に設定(毎日)
- 夕方の投稿枠 → 投稿予定日を「17:00」に設定(平日のみ。土日にこの枠のネタを予約しても、
  夕方のワークフロー自体が動かないため投稿されません)

## コンテンツポリシー

- 未公開の選考情報・社内限定情報などのインサイダー情報は一切使用しない
- 景品表示法・個人情報保護法・著作権など関連法令を遵守した内容にする
- 体験談等を扱う場合は、個人が特定されない形に配慮する

新しいネタを作成する際は、上記を前提に作成します。
